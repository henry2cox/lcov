...
sub isOutOfRange {
  ...
  if (defined($self->[2]) &&
    ...
    # can also be caused by gcov extract bug

    my $filt = $lcovutil::cov_filter[$lcovutil::FILTER_LINE_RANGE];
    if (defined($filt)) {
      lcovutil::verbose("exclude out-of-range line $lineNo in " .
                        $self->filename() . " (" .
    ...
    if (lcovutil::warn_once($key)) {
      my $msg = "unknown line '$lineNo' in " . $self->filename()
    ...
    foreach ($line)
    {
      ...
      /^DA:(\d+),(-?\d+)(,[^,\s]+)?/ && do
      {
        ...
        next if ($readSourceCallback->notEmpty() &&
                 $readSourceCallback->isOutOfRange($line));
  ...
  foreach my $source_file (sort($self->files())) {
    ...
    foreach my $testname (sort($testdata->keylist())) {
      ...
      foreach my $line (sort({$a <=> $b} $testcount->keylist())) {
        next
          if (defined($reader) &&
              ($reader->isOutOfRange($line) || $reader->isExcluded($line)));
